-- originally by https://github.com/AllenDang/nvim-expand-expr
...
function M.expand()
  ...
  if pos == nil then
    vim.notify("invalid expand expr, no character '|' found", vim.log.levels.ERROR)
    return
  end
  ...
  if #range == 0 and #content == 0 then
    vim.notify("invalid expandable expr", vim.log.levels.ERROR)
    return
  end
